...
export class TrainScraper {
  ...
  buildUrl(fromStation: string, toStation: string, travelDate: string, departureTime: string): string {
  ...
  extractTrainNumber(text: string): string {
    const match = text.match(/Train number:\s*(\S+)/i);
    return match ? match[1] : text.replace('Train number:', '').trim();
  }

  extractStopCount(text: string): number {
    const match = text.match(/(\d+)/);
    return match ? Number(match[1]) : 0;
  }

  formatStop(stop: Stop): string {
    return `${stop.time} - ${stop.station} - ${stop.type}`;
  }

  extractData($: any, card: any): TrainData[] {
    const data: TrainData[] = [];
    let currentData: TrainData | null = null;

    $(card).find('.flex.items-center').each((_index:any, element:any) => {
    ...
    stationRows.each((_index: any, row:any) => {
  ...
  formatScheduleData(schedule: Schedule, index: number): string {
    const parts = [
      `\n------- option ${index} ------\n`,
      schedule.line,
  ...
  parseScheduleCard($: any, card: any): Schedule | null {
    const optionText = $(card).find('h2').first().text().trim();
    const transferText = $(card).find('p, span, div').filter((_index: any, el:any) => {
  ...
  async getTrainSchedule(
    ...
    try {
      ...
      $('div.bg-white.rounded-lg.shadow-lg').each((_index, card) => {
      ...
      console.error('Error fetching schedule:', error instanceof Error ? error.message : 'Unknown error');
      return [];
    }
  }
}
